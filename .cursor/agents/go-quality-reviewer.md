...
- **Test placement:** all tests under `test/` only (no `*_test.go` in `cmd/`, `internal/`, `pkg/`). Unit tests: `test/unit/pkg/...` import `pkg/` only. Blackbox: `test/blackbox/...` exec compiled `cmd/guesschema`, no `pkg`/`internal` imports.
